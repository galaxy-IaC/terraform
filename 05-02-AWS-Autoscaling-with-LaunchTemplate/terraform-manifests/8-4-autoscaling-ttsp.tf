...
resource "aws_autoscaling_policy" "avg_cpu_policy_greater_than_xx" {
  ...
  # could pick three the policy type such like SimpleScaling/StepScaling/TargetTrackingScaling
  # if we specify others will default to "SimpleScaling"
...
resource "aws_autoscaling_policy" "alb_target_requests_greater_than_yy" {
  ...
  # Number of requests greater than 10 per target in ALB target group.
  target_tracking_configuration {
    predefined_metric_specification {
